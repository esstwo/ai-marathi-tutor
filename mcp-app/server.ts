 ...
 *
 * Auth (HTTP mode only):
 *   OAuth 2.1 + PKCE over Supabase email/password.
 *   Claude discovers /.well-known/oauth-authorization-server, runs the browser
 *   flow, and then sends Authorization: Bearer <supabase-jwt> on every /mcp
 *   request. requireAuth validates the JWT and attaches parentId to the session.
 */
...
// ── MCP Server factory ───────────────────────────────────────────────
// Each session gets its own McpServer + MarathiApiClient so auth tokens
// are scoped per-session and never shared across connections.

function createServer(api: MarathiApiClient): McpServer {
  const server = new McpServer({ name: "MarathiMitra", version: "1.0.0" });
  ...
  server.resource(
    ...
    async () => ({
      contents: [{ uri: "ui://marathi-mitra/conversation", mimeType: "text/html;profile=mcp-app", text: loadAppHtml("conversation") }],
    })
  );
  ...
  server.resource(
    ...
    { mimeType: "text/html;profile=mcp-app" },
    async () => ({
      contents: [{ uri: "ui://marathi-mitra/lessons", mimeType: "text/html;profile=mcp-app", text: loadAppHtml("lessons") }],
    })
  );
  ...
  server.resource(
    ...
    { mimeType: "text/html;profile=mcp-app" },
    async () => ({
      contents: [{ uri: "ui://marathi-mitra/progress", mimeType: "text/html;profile=mcp-app", text: loadAppHtml("progress") }],
    })
  );
  ...
  server.registerTool("start-marathi-practice", {
    ...
    description: "Start a Marathi conversation with Mitra, the AI tutor. Opens an interactive chat UI.",
    inputSchema: { child_id: z.string().describe("The child's UUID") },
    _meta: { ui: { resourceUri: "ui://marathi-mitra/conversation" } },
  }, async ({ child_id }) => {
    try {
  ...
  server.registerTool("browse-lessons", {
    ...
    description: "Browse Marathi vocabulary lessons by level. Opens a lesson browser with flashcards and quizzes.",
    inputSchema: {
    ...
    _meta: { ui: { resourceUri: "ui://marathi-mitra/lessons" } },
  }, async ({ child_id, level }) => {
    try {
      const targetLevel = level || 1;
      const lessons = await api.listLessons(targetLevel);
      return { content: [{ type: "text" as const, text: JSON.stringify({ child_id, level: targetLevel, lessons }) }] };
    } catch (e: unknown) {
      return { content: [{ type: "text" as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
  });
  ...
  server.registerTool("show-progress", {
    title: "Show Progress",
    description: "Show a child's Marathi learning progress — XP, streak, level, lessons completed.",
    inputSchema: { child_id: z.string().describe("The child's UUID") },
    _meta: { ui: { resourceUri: "ui://marathi-mitra/progress" } },
  }, async ({ child_id }) => {
    try {
  ...
  server.registerTool("chat-end", {
    ...
    inputSchema: { conversation_id: z.string().describe("Active conversation UUID") },
  ...
  server.registerTool("speak-marathi", {
    ...
    inputSchema: { text: z.string().max(200).describe("Marathi text to speak (max 200 chars)") },
  }, async ({ text }) => {
    try {
      const base64Audio = await api.speakMarathi(text);
      return { content: [{ type: "text" as const, text: JSON.stringify({ audio_base64: base64Audio }) }] };
    } catch (e: unknown) {
      return { content: [{ type: "text" as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
  });
  ...
  server.registerTool("list-lessons", {
    ...
    inputSchema: { level: z.number().min(1).max(5).describe("Lesson level (1-5)") },
  }, async ({ level }) => {
    try {
      const lessons = await api.listLessons(level);
      return { content: [{ type: "text" as const, text: JSON.stringify(lessons) }] };
    } catch (e: unknown) {
      return { content: [{ type: "text" as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
  });
  ...
  server.registerTool("get-lesson", {
    ...
    try {
      ...
      return { content: [{ type: "text" as const, text: JSON.stringify(lesson) }] };
    } catch (e: unknown) {
      return { content: [{ type: "text" as const, text: `Error: ${e instanceof Error ? e.message : String(e)}` }], isError: true };
    }
  });
  ...
  server.registerTool("get-progress", {
    ...
    inputSchema: { child_id: z.string().describe("Child UUID") },
  }, async ({ child_id }) => {
    try {
      const progress = await api.getProgress(child_id);
      return { content: [{ type: "text" as const, text: JSON.stringify(progress) }] };
...
// ── Transport ────────────────────────────────────────────────────────
// --stdio flag: single session for Claude Desktop (no auth needed, uses service key).
// Default: HTTP mode with OAuth 2.1 for remote/claude.ai access.

const useStdio = process.argv.includes("--stdio");

if (useStdio) {
  const api = new MarathiApiClient(API_BASE_URL, { serviceKey: SERVICE_KEY });
  ...
  expressApp.post("/token", tokenExchange);

  // ── MCP session management (auth required) ───────────────────────

  type Session = { transport: StreamableHTTPServerTransport; server: McpServer };
  const sessions = new Map<string, Session>();

  expressApp.post("/mcp", requireAuth, async (req, res) => {
    ...
    // New session — create a per-user API client with the validated JWT
    const api = new MarathiApiClient(API_BASE_URL, { userToken: authReq.userToken });
    const mcpServer = createServer(api);
    ...
    if (newSessionId) sessions.set(newSessionId, { transport, server: mcpServer });
  });

  expressApp.get("/mcp", requireAuth, async (req, res) => {
    const sessionId = req.headers["mcp-session-id"] as string | undefined;
    if (sessionId && sessions.has(sessionId)) {
      const { transport } = sessions.get(sessionId)!;
  ...
  expressApp.delete("/mcp", requireAuth, async (req, res) => {
    const sessionId = req.headers["mcp-session-id"] as string | undefined;
    if (sessionId && sessions.has(sessionId)) {
      const { transport } = sessions.get(sessionId)!;
      await transport.handleRequest(req, res);
  ...
  expressApp.listen(PORT, () => {
    console.log(`MarathiMitra MCP App server running on port ${PORT}`);
